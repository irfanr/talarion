...
@Profile({ Constants.SPRING_PROFILE_METRICS, Constants.SPRING_PROFILE_PRODUCTION })
public class MetricsConfiguration extends MetricsConfigurerAdapter implements EnvironmentAware {
  ...
  public static class GraphiteRegistry implements EnvironmentAware {
    ...
    @PostConstruct
    private void init() {
      Boolean graphiteEnabled = propertyResolver.getProperty(PROP_GRAPHITE_ENABLED, Boolean.class,
          false);
      if (graphiteEnabled) {
        ...
        String graphitePrefix = propertyResolver
            .getProperty(PROP_GRAPHITE_PREFIX, String.class, "");
